...
func NewRestore(snapshotName, backupURL, currentRestoringBackup string) *RestoreStatus {
...
func (status *RestoreStatus) OpenVolumeDev(volDevName string) (*os.File, string, error) {
	volumePath := GetSnapshotPath(volDevName)
	if _, err := os.Stat(volumePath); os.IsNotExist(err) {
		logrus.Errorf("dev file %s is not exist", volumePath)
		return nil, volumePath, err
	}

	file, err := os.Open(volumePath)
	return file, volumePath, err
}
